...
var _alastG=null, _alastT=0;
gd.on('plotly_click', function(e){
  if(mode!=='analogy') return;
  var p=e.points[0]; if(p.curveNumber>=base) return;
  var g=p.customdata; if(g==null) return;
  var now=+new Date();
  if(g===_alastG && now-_alastT<400) return;   // ignore gl3d double-fire
  _alastG=g; _alastT=now;
  if(alist.length>=4) alist=[];
  alist.push(LABELS[g]); renderAList();
});
bD.addEventListener('click', function(){ renderAList(); });
bA.addEventListener('click', function(){ alist=[]; renderAList(); });
renderAList();
"""
